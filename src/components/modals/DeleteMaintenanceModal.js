...
import { deleteMaintenance } from '../../utils/CRUD.services';

const DeleteMaintenanceModal = ({ onchange, data }) => {
  const [showModal, setShowModal] = useState(true);
  const [enteredConfirmData, setEnteredConfirmData] = useState();

  const disabledBtn = enteredConfirmData && "DELETE MAINTENANCE" === enteredConfirmData ? false : true;
  ...
  const saveChanges = async () => {
    const response = await deleteMaintenance(data.maintenance_id);
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t">
                  <h3 className="text-3xl font-semibold">
                    Delete maintenance info <i className="zmdi zmdi-delete ml-2 text-red-500"></i>
                  </h3>
                </div>
                {/*body*/}
                <div className="relative p-6 flex-auto">
                  <p className="my-4 text-slate-500 text-lg leading-relaxed">
                    Delete this maintenance information? <br />
                    Enter <span className='text-red-500 font-bold'>DELETE MAINTENANCE</span> to confirm
                  </p>
